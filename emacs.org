...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar wheel/default-font-size 120)
  (defvar wheel/default-variable-font-size 120)

  ;; Make frame transparency overridable
  (defvar wheel/frame-transparency '(60 . 60))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun wheel/display-startup-time ()
  ...
  (add-hook 'emacs-startup-hook #'wheel/display-startup-time)
...
** Text Edit
*** Multi Edit
    ref [[https://sam217pa.github.io/2016/09/11/nuclear-power-editing-via-ivy-and-ag/][Nuclear weapon multi-editing via Ivy and Ag]]
    =counsel-rg= search string I want to modify in current directory.
    =C-c C-o(ivy-occur)= in search result. It opens an ivy-occur buffer.
    =C-x C-q(ivy-wgrep-change-to-wgrep-mode)= edit it.
    usee normal search and replace techniques, like Evil-ex commands =:%s/pattern/replace/g=
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-frame-parameter (selected-frame) 'alpha wheel/frame-transparency)
  (add-to-list 'default-frame-alist `(alpha . ,wheel/frame-transparency))
...
(set-face-attribute 'default nil :font "Fira Code Retina" :height wheel/default-font-size)

;; Set the fixed pitch face
(set-face-attribute 'fixed-pitch nil :font "Fira Code Retina" :height wheel/default-font-size)

;; Set the variable pitch face
(set-face-attribute 'variable-pitch nil :font "Cantarell" :height wheel/default-variable-font-size :weight 'regular)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package general
    :after evil
    :config
    (general-create-definer wheel/leader-keys
    ...
    (wheel/leader-keys
      "/" '(swiper :wk "swiper")
      "g" '(magit :wk "magit")
      "s" '(search-files/body :wk "search")
      "p" '(:ignore t :wk "projectile")
      "pf" '(projectile-find-file-in-known-projects :wk "file")
      "pb" '(counsel-projectile-switch-to-buffer :wk "buffer")
      "pd" '(counsel-projectile-find-dir :wk "dir")
      "pp" '(counsel-projectile-switch-project :wk "project")
      "b" '(hydra-buffer/body :wk "buffer")
      "t" '(hydra-toggle/body :wk "toggles")
      "w" '(hydra-window/body :wk "window")
      ;;      "tt" '(counsel-load-theme :wk "choose theme")
      "f" '(:ignore t :wk "files")
      "fe" '(lambda () (interactive) (find-file (expand-file-name "~/.emacs.d/emacs.org")) :wk "emacs.org")
  ...
  (defhydra hydra-toggle (:color pink :hint nil)
    ...
    _c_ command-log-mode:  %`command-log-mode
    _d_ debug-on-error:    %`debug-on-error
    _f_ auto-fill-mode:    %`auto-fill-function
    _h_ highlight          %`highlight-nonselected-windows
    _t_ truncate-lines:    %`truncate-lines
    _w_ show-trailing-whitespace:    %`show-trailing-whitespace
...
;  ("w" whitespace-mode)
					  ...
					  ;(if (boundp 'command-log-mode)
					  ;    (clm/open-command-log-buffer)
					  ;  (progn (setq command-log-mode t)
					  ;         (clm/open-command-log-buffer)))
  ...
  (use-package evil-escape
    ...
    (setq-default evil-escape-delay 0.11)
    (evil-escape-mode))
#+END_SRC
...
(use-package org
  ...
  :hook (org-mode . wheel/org-mode-setup)
  :config
  ...
  (setq org-todo-keywords
    '((sequence "TODO(t)" "DOING(d!)" "|" "CLOSE(C!)")
  ...
  (setq org-capture-templates
    `(("t" "Tasks / Projects")
      ("tt" "Task" entry (file+olp "~/doc/todo.org" "Inbox")
           "* TODO %?\n  %U\n  %a\n  %i" :empty-lines 1)

      ("j" "Journal Entries")
      ("jj" "Journal" entry
           (file+olp+datetree "~/doc/journal.org")
      ...
      ("w" "Workflows")
      ("we" "Checking Email" entry (file+olp+datetree "~/doc/journal.org")
           "* Checking Email :email:\n\n%?" :clock-in :clock-resume :empty-lines 1)

      ("m" "Metrics Capture")
      ("mw" "Weight" table-line (file+headline "~/metrics.org" "Drink")
       "| %U | %^{Weight} | %^{Notes} |" :kill-buffer t)))

  (define-key global-map (kbd "C-c j")
    (lambda () (interactive) (org-capture nil "jj")))

  (wheel/org-font-setup))

#+END_SRC
** Better Font Faces

The =wheel/org-font-setup= function configures various text faces to tweak the sizes of headings and use variable width fonts in most cases so that it looks more like we're editing a document in =org-mode=.  We switch back to fixed width (monospace) fonts for code blocks and tables so that they display correctly.

#+BEGIN_SRC emacs-lisp

(defun wheel/org-font-setup ()
...
This snippet adds a hook to =org-mode= buffers so that =wheel/org-babel-tangle-config= gets executed each time such a buffer gets saved.  This function checks to see if the file being saved is the Emacs.org file you're looking at right now, and if so, automatically exports the configuration here to the associated output files.

#+BEGIN_SRC emacs-lisp

  ;; Automatically tangle our Emacs.org config file when we save it
  (defun wheel/org-babel-tangle-config ()
  ...
  (add-hook 'org-mode-hook (lambda () (add-hook 'after-save-hook #'wheel/org-babel-tangle-config)))
...
#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'org
    (progn
      ...
      (add-to-list 'org-structure-template-alist '("sh" "#+BEGIN_SRC python\n?\n#+END_SRC"))
      )
    )
#+END_SRC
...
** Center Org Buffers
#+BEGIN_SRC emacs-lisp
(defun wheel/org-mode-visual-fill ()
  (setq visual-fill-column-width 100
        visual-fill-column-center-text t)
  (visual-fill-column-mode 1))

(use-package visual-fill-column
  :hook (org-mode . wheel/org-mode-visual-fill))
#+END_SRC
* Development
** Languages
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun wheel/lsp-mode-setup ()
    (setq lsp-headerline-breadcrumb-segments '(path-up-to-project file symbols))
    (lsp-headerline-breadcrumb-mode))

  (use-package lsp-mode
    :commands (lsp lsp-deferred)
    :hook (lsp-mode . wheel/lsp-mode-setup)
    :init
...
#+BEGIN_SRC emacs-lisp
  (use-package geiser
    :config
    (setq geiser-scheme-binary "/usr/bin/mit-scheme"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun wheel/configure-eshell ()
    ...
    (evil-define-key '(normal insert visual) eshell-mode-map (kbd "C-r") 'counsel-esh-history)k
  ...
  (use-package eshell
    :hook (eshell-first-time-mode . wheel/configure-eshell)
    :config
...
(
    ...
    ("@" ag "Search With Ag")
    ("|" ag-file "Ag Search Text in Files")
